...
test('protected: audit-events.md', () => {
  assert.equal(sd.isProtectedPath('/proj/security/audit-events.md'), true);
});

test('protected: code audit state files', () => {
...
test('protected via parent symlink: parent dir is symlinked', () => {
  // .../proj-link/security/dependency-events.md where proj-link points to real-proj.
  const dir = tmpDir();
