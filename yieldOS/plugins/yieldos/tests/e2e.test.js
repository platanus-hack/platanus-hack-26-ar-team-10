...
test('npm install of allowlisted package passes (exit 0)', () => {
  ...
  assert.equal(hookContext(r).includes('🛡  yieldOS  ·  Validado'), true);
});
...
test('npm install of bcrypt (Category D) blocks', () => {
  ...
  assert.equal(hookContext(r).includes('Bloqueado · categoría crítica'), true);
});
...
test('Write to protected file is blocked by self-defense', () => {
  ...
  assert.equal(hookContext(r).includes('Bloqueado · archivo protegido'), true);
});
...
test('CLAUDE.md edit with injection pattern blocks', () => {
  ...
  assert.equal(r.code, 2);
  assert.equal(hookContext(r).includes('Bloqueado · inyección detectada'), true);
});
