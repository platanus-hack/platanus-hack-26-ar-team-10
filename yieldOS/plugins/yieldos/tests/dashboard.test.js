...
test('server: GET /assets/app.js disables stale browser cache', async () => {
  const root = tmpProject();
  const { server, port } = await dashboardServer.start({ projectRoot: root, port: 0 });
  try {
    const res = await fetchOnce(port, '/assets/app.js');
    assert.equal(res.status, 200);
...
test('server: dashboard port parser is bounded', () => {
  assert.equal(dashboardServer.parsePort('0'), 0);
  assert.equal(dashboardServer.parsePort('5473'), 5473);
  assert.equal(dashboardServer.parsePort('99999'), 5473);
  assert.equal(dashboardServer.parsePort('nope'), 5473);
  assert.equal(launcher.parsePort('5480'), 5480);
  assert.equal(launcher.parsePort('99999'), 5473);
});
