# Hackathon Pitch: Oracle-Driven yieldOS
...
yieldOS is an oracle-driven security harness for AI coding agents. The model can propose. The oracle decides. Before risky changes are accepted, yieldOS asks executable checks for scoped `pass`, `fail`, or `unknown` evidence. `unknown` blocks sensitive actions by default.

## Problem

Most tools either ask the model to reason harder or scan after the fact. That still leaves the user trusting an opinion. yieldOS turns risky actions into acceptance decisions with evidence.
...
- Oracles now normalize existing checks into scoped pass/fail/unknown results.
- CDSC demo proves one concrete class: public admin routes must reject unauthenticated requests.
...
3. yieldOS detects `missing-authz` and creates a contract.
4. Replay against the vulnerable runtime gets `200`.
