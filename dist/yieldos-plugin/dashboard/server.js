...
function parsePort(value) {
  ...
  if (!Number.isInteger(port) || port < 0 || port > 65535) return 5473;
  return port;
}
...
function safeJoinPublic(reqPath) {
  let cleaned;
  try {
    cleaned = decodeURIComponent(reqPath).replace(/^\/+/, '');
  } catch (_) {
    return null;
  }
...
module.exports = { createServer, start, eventsPath, readAllEvents, attachLiveTail, parsePort };
